...
This project uses Tailwind CSS CLI for production-ready styling.
...
**Important:** Always run the build command before deploying to production. The CDN version should not be used in production environments.

### Database Schema Updates

If you encounter SQL errors related to `action_participants` table, ensure you've run the latest schema updates:
```bash
mysql -u your_user -p your_database < model/schema.sql
```

### Directory Structure Updates

The following new directories were created to organize dashboard assets:
- `dashboard/assets/` - Main assets directory
- `dashboard/assets/js/` - JavaScript files including `countries.js`
- `dashboard/input.css` - Tailwind CSS input file
- `dashboard/output.css` - Compiled Tailwind CSS (generated by build process)

The `countries.js` file contains comprehensive country data (name, code, aliases) sorted alphabetically for use in action/resource creation forms.
...
### Overview
The platform uses **Resend API** for sending transactional emails to users for all system events.

### Email Events
- **Action Approved/Rejected**: Sent to action creator when admin approves/rejects their action
- **Resource Approved/Rejected**: Sent to resource publisher when admin approves/rejects their resource
- **Comment Added**: Sent to action/resource creator when someone comments
- **Report Created**: Sent to admins when a new report is submitted
- **Report Status Updated**: Sent to reporter when their report status changes
- **Reminder Due**: Sent to users 24 hours before their action/resource event

### Configuration
- API Key is read from environment variable `RESEND_API_KEY`
- Email service is implemented in `utils/EmailService.php` using cURL
- All emails are sent from `noreply@connectforpeace.com`
...
For **Windows (XAMPP)**:
1. Set the environment variable in Windows:
   - Open System Properties → Advanced → Environment Variables
   - Add new system variable: `RESEND_API_KEY` with your actual API key
   - Or set it for your user account only
2. Restart Apache after setting the variable
...
1. Add to your shell profile (`~/.bashrc`, `~/.zshrc`, etc.):
   ...
   ```
2. Run `source ~/.bashrc` (or appropriate file) to reload

For **Apache Configuration** (alternative approach):
1. Add to your Apache Virtual Host or `.htaccess`:
   ```
   SetEnv RESEND_API_KEY "your_actual_api_key_here"
   ```

### Development Mode
- When `DEVELOPMENT_MODE` is set to `true` in `config/config.php` and `RESEND_API_KEY` is not set, the system will log a warning but continue to operate
- In production mode, the application will fail fast with a clear error if the API key is not set

### Testing
- Check Apache error logs for email sending status: `D:\xampp\apache\logs\error.log`
- Verify Resend API dashboard for email delivery status

## Automated Reminders (Cron Job Setup)

### Overview
The platform automatically sends reminder emails and notifications 24 hours before scheduled actions/resources.
...
3. Name: "Connect for Peace Reminders"
4. Trigger: **Daily** at 12:00 AM
5. Action: **Start a program**
6. Program: `C:\xampp\php\php.exe`
7. Arguments: `C:\xampp\htdocs\my_work_v5\my_work_v2\utils\send_reminders.php`
8. Click **Finish**
...
2. Add this line:
   ```
   */15 * * * * /usr/bin/php /path/to/my_work_v2/utils/send_reminders.php >> /path/to/logs/reminders.log 2>&1
   ```
3. Save and exit

### How It Works
1. Cron job runs `utils/send_reminders.php` every 15 minutes (or daily)
2. Script fetches all due reminders from database (where `reminder_time <= NOW()` and `sent = 0`)
3. For each reminder:
   - Sends email via Resend API (if `reminder_type` is 'email' or 'both')
   - Creates in-app notification (if `reminder_type` is 'in_app' or 'both')
   - Marks reminder as sent in database
4. Logs success/failure for each reminder

### Testing
- Manually run: `php C:\xampp\htdocs\my_work_v5\my_work_v2\utils\send_reminders.php`
- Check output for "Cron job completed. Processed X reminders."
- Check Apache error logs for detailed processing logs
